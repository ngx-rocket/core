...
class CoreGenerator extends Generator {
   ...
   * @return `true` if the generator is running standalone or `false` if it is running as an add-on.
   */
  get isStandalone() {
   ...
   * @return `true` if this or a composed generator has declared to be in `server` or `fullstack` mode or `false` if it
   ...
   * @return Returns the package manager to use (either `npm` or `yarn`)
   */
  get packageManager() {
  ...
  writing() {
    ...
    if (process.env.NGX_CLIENT_PATH === null || process.env.NGX_CLIENT_PATH === undefined) {
      process.env.NGX_CLIENT_PATH = 'client';
    }
    if (process.env.NGX_SERVER_PATH === null || process.env.NGX_CLIENT_PATH === undefined) {
      process.env.NGX_SERVER_PATH = 'server';
    }

    return FileUtilities.getFiles(this._templatesPath, this._type)
      .then(allFiles => {
        let files = FileUtilities.prepareFiles(
          allFiles.client,
          this._type === 'fullstack' ?
            path.join(this._templatesPath, FileUtilities.ClientTemplatesPath) : this._templatesPath,
          this.isFullstack ? process.env.NGX_CLIENT_PATH : null
        );
        files = files.concat(FileUtilities.prepareFiles(
          allFiles.server,
          this._type === 'fullstack' ?
            path.join(this._templatesPath, FileUtilities.ServerTemplatesPath) : this._templatesPath,
