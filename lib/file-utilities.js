...
class FileUtilities {
  ...
  static prepareFiles(files, templatePath) {
    ...
    return _.map(files, file => {
      ...
      const folderConditions = this._getConditions(dirname, '__');
      const actionMatch = FileActionRegExp.exec(basename);
      const action = actionMatch ? actionMatch[1] : null;
      let isTemplate = basename.startsWith('_');
      let dest = path.relative(folderConditions.length ? dirname.split(path.sep)[0] : '.', src);
  ...
  static _getConditions(name, prefix) {
    if (name.startsWith(prefix)) {
      ...
      return name.substring(prefix.length, dotIndex).split('+');
    }
    return [];
  }
