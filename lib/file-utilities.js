...
class FileUtilities {
  ...
  static getFiles(templatePath, generatorType) {
    let promises;
  ...
  static prepareFiles(files, templatePath, destinationPath) {
  ...
  static filterFiles(files, patterns) {
    ...
    return files.filter(file => !filter.ignores(path.relative(path.base || '', file.dest)));
  }
